...
To validate the JSON schema for POST request
    Background: Create and initialize base url
      ...
      * def body = read("../requestSchemas/postSchemaAddJob.json")
      ...
      When method POST
      And status 201
      * string jsonSchemaExpected = read('../requestSchemas/dataSchema.json')
      And print 'EXPECTED SCHEMA', jsonSchemaExpected
      * string jsonData = response
       And print 'RESPONSE', jsonData
      * def SchemaUtils = Java.type('baseapi.utils.JSONSchemaUtil')
